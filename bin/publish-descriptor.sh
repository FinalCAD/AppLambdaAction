...
function main.exec() {
  ...
  check.env \
    ...
    DEPLOYMENT_DESCRIPTOR_METADATA_IS_ARCHIVE \
    DEPLOYMENT_DESCRIPTOR_POLICIES_FILE \
  ...
  local descriptor_directory="${DEPLOYMENT_DESCRIPTOR_GIT_REPOSITORY}/lambda/${FINALCAD_ENVIRONMENT}/${FINALCAD_REGION_FRIENDLY}/${application_name}"
  ...
  if [[ "${DEPLOYMENT_DESCRIPTOR_METADATA_IS_ARCHIVE}" == "true" ]]; then
    TERRAFORM_APP_LAMBDA_PACKAGE_TYPE='Archive'
  else
    TERRAFORM_APP_LAMBDA_PACKAGE_TYPE='Image'
  fi
...
export TERRAFORM_APP_LAMBDA_PACKAGE_TYPE='${TERRAFORM_APP_LAMBDA_PACKAGE_TYPE}'
EOF
  {
    rc=$?
  ...
  jq \
    ...
    --arg metadata_application_is_archive "${DEPLOYMENT_DESCRIPTOR_METADATA_IS_ARCHIVE}" \
    --arg metadata_application_package_type "${TERRAFORM_APP_LAMBDA_PACKAGE_TYPE}" \
    --arg metadata_application_inject_route_files "${DEPLOYMENT_DESCRIPTOR_METADATA_INJECT_ROUTE_FILES}" \
    ...
    '{
      application: {
        ...
        inject_route_files: $metadata_application_inject_route_files,
        is_archive: $metadata_application_is_archive,
        package_type: $metadata_application_package_type,
      },
      terraform: {
